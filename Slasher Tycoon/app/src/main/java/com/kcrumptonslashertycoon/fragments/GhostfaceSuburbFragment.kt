...
class GhostfaceSuburbFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        return inflater.inflate(R.layout.fragment_ghostface_suburb, container, false)
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        view.findViewById<Button>(R.id.buttonStore).setOnClickListener {
            val action = GhostfaceSuburbFragmentDirections
                .actionGhostfaceSuburbFragmentToStoreFragment("Suburbs")
                        ...
                        lifecycleScope.launch {
                            ...
                            val dialog =
                                SelectInventoryItemDialogFragment(inventoryList) { selectedItem ->
                                    lifecycleScope.launch {
                                        if (currentTask != null) {
                                            handleItemUsed(selectedItem, currentTask!!.taskMessage)
                                        } else {
        ...
        lifecycleScope.launch {
            slasherViewModel.getActiveTask(userId, slasherId = 3).collect { task ->
                currentTask = task
                bannerTextView.text = task?.taskMessage ?: "Try something else or go back and wait for more tasks."
                bannerTextView.setBackgroundColor(
                    requireContext().getColor(
                        if (task != null) R.color.task_active_ghostface
                        else R.color.task_inactive_ghostface
                    )
                )
            }
        }

        val roomName = "Suburbs"

        lifecycleScope.launchWhenStarted {
            slasherViewModel.slashers.collect { slashers ->
                val ghostface = slashers.find { it.name == "Ghostface" }
                if (ghostface != null) {
                    visitorTextView.text = "Visitor Count: ${ghostface.visitorCount}"
                    killsTextView.text = "Kills Today: ${ghostface.killsToday}"

                    val prevCount = slasherViewModel.getLastKnownVisitorCount(roomName)
                    val gained = ghostface.visitorCount - prevCount

                    if (gained > 0) {
                        slasherViewModel.updateLastKnownVisitorCount(
                            roomName,
                            ghostface.visitorCount
                        )
    ...
    private fun handleItemUsed(item: UserInventoryItem, taskMessage: String) {
        ...
        lifecycleScope.launch {
            val task = slasherViewModel.getActiveTask(userId, slasherId = 3).firstOrNull()
            ...
            if (task != null && !task.isCompleted) {
                ...
                if (keyword.isBlank() || itemName.contains(keyword)) {
                    ...
                    val allEffects =
                        slasherViewModel.getRoomEffects("Suburbs").firstOrNull() ?: listOf()
                    ...
                    if (currentUser != null) {
                        ...
                        withContext(Dispatchers.Main) {
                            val floatingText = TextView(requireContext()).apply {
                                ...
                                setShadowLayer(3f, 2f, 2f, Color.parseColor("#000000"))
                            }
                        ...
                        slasherViewModel.addKillsToSlasher(slasherId = 3, killsToAdd = totalKills)
                        storeViewModel.removeItemFromInventory(item)

                        // Visitor and Kill counts applied to the slasher
                        val slasher =
                            slasherViewModel.slashers.value.find { it.locationType == "Suburbs" }
                        if (slasher != null) {
                        ...
                        withContext(Dispatchers.Main) {
                            view?.let { rootView ->
                                val snackbar = Snackbar.make(
                                    rootView,
                                    "That item won't help Ghostface with this task.",
    ...
    private suspend fun handleFreePlayItemUse(item: UserInventoryItem) {
        ...
        val slasher =
            slasherViewModel.slashers.value.find { it.locationType == "Suburbs" } ?: return

        // Generate task
        val task = FreeplayManager.getTaskForItem(item.itemName, userId, slasherId = 3, userLevel)

        if (task == null) {
            view?.let { rootView ->
                val snackbar = Snackbar.make(
                    rootView,
                    "Ghostface yawns. That didn't help.",
                    Snackbar.LENGTH_SHORT
                )
                val snackbarView = snackbar.view
        ...
        val effects = slasherViewModel.getRoomEffects("Suburbs").firstOrNull() ?: listOf()
