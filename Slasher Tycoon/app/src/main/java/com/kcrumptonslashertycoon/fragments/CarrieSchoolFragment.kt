...
class CarrieSchoolFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        return inflater.inflate(R.layout.fragment_carrie_school, container, false)
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        view.findViewById<Button>(R.id.buttonStore).setOnClickListener {
            val action = CarrieSchoolFragmentDirections
                .actionCarrieSchoolFragmentToStoreFragment("High School")
        ...
        // Shaky task text
        //val levelText = view.findViewById<TextView>(R.id.taskBannerTextView)
        //val jitterAnim = AnimationUtils.loadAnimation(requireContext(), R.anim.jitter)
        //levelText.startAnimation(jitterAnim)

        // Pulsing ceiling light
        val glow = view.findViewById<ImageView>(R.id.hangingLightGlow)
        val flicker = AnimationUtils.loadAnimation(requireContext(), R.anim.flicker)
        glow.startAnimation(flicker)

        // Had to look this up - making the glow for the light always be in the right spot on any phone
        val background = view.findViewById<ImageView>(R.id.carrieBackground)

        background.viewTreeObserver.addOnGlobalLayoutListener {
            val drawable = background.drawable ?: return@addOnGlobalLayoutListener
            ...
            // 🔹 tweak these numbers visually
            val xPercent = 0.63f
            val yPercent = 0.13f

            val drawnWidth = imageWidth * scaleX
            val drawnHeight = imageHeight * scaleY

            val finalX = transX + drawnWidth * xPercent
            val finalY = transY + drawnHeight * yPercent

            glow.x = finalX - glow.width / 2f
            glow.y = finalY - glow.height / 2f
            ...
            slasherViewModel.getUserInfo(userId).collect { userInfo ->
                userInfo?.let {
                    ...
                    storeViewModel = ViewModelProvider(
                        this@CarrieSchoolFragment,
                        StoreViewModelFactory(requireContext(), "High School", userLevel)
        ...
        lifecycleScope.launch {
            slasherViewModel.getActiveTask(userId, slasherId = 7).collect { task ->
                currentTask = task
                if (task != null) {
                    bannerTextView.text = task.taskMessage
                    bannerTextView.setBackgroundColor(requireContext().getColor(R.color.task_active_carrie))
                } else {
                    bannerTextView.text = "No tasks right now. Look around or go back."
                    bannerTextView.setBackgroundColor(requireContext().getColor(R.color.task_inactive_carrie))
                }
            }
        }

        val roomName = "High School"

        lifecycleScope.launchWhenStarted {
            slasherViewModel.slashers.collect { slashers ->
                val carrie = slashers.find { it.name == "Carrie" }
                if (carrie != null) {
                    visitorTextView.text = "Visitor Count: ${carrie.visitorCount}"
                    killsTextView.text = "Kills Today: ${carrie.killsToday}"

                    val prevCount = slasherViewModel.getLastKnownVisitorCount(roomName)
                    val gained = carrie.visitorCount - prevCount

                    if (gained > 0) {
                        slasherViewModel.updateLastKnownVisitorCount(roomName, carrie.visitorCount)
                        ...
                        if (user != null) {
                            ...
                            val floatingText = TextView(requireContext()).apply {
                                text = "+${gained}"
                                textSize = 52f
                                setTextColor(Color.parseColor("#FFD84D"))
    ...
    private fun handleItemUsed(item: UserInventoryItem, taskMessage: String) {
        ...
        lifecycleScope.launch {
            val task = slasherViewModel.getActiveTask(userId, slasherId = 7).firstOrNull()
            ...
            if (task != null && !task.isCompleted) {
                ...
                if (keyword.isBlank() || itemName.contains(keyword)) {
                    ...
                    val allEffects =
                        slasherViewModel.getRoomEffects("High School").firstOrNull() ?: listOf()
                    ...
                    slasherViewModel.addKillsToSlasher(slasherId = 7, killsToAdd = totalKills)
                    storeViewModel.removeItemFromInventory(item)

                    // Visitor and Kill counts applied to the slasher
                    val slasher =
                        slasherViewModel.slashers.value.find { it.locationType == "High School" }
                    if (slasher != null) {
                        ...
                        Log.e("CarrieDebug", "Couldn't find Carrie in slasher list by location.")
                    }
                    ...
                    withContext(Dispatchers.Main) {
                        view?.let { rootView ->
                            val snackbar = Snackbar.make(
                                rootView,
                                "That item won't help Carrie with this task.",
    ...
    private suspend fun handleFreePlayItemUse(item: UserInventoryItem) {
        ...
        val slasher =
            slasherViewModel.slashers.value.find { it.locationType == "High School" } ?: return

        // Generate task
        val task = FreeplayManager.getTaskForItem(item.itemName, userId, slasherId = 7, userLevel)

        if (task == null) {
            view?.let { rootView ->
                val snackbar = Snackbar.make(
                    rootView,
                    "Carrie yawns. That didn't help.",
                    Snackbar.LENGTH_SHORT
                )
                val snackbarView = snackbar.view
        ...
        val effects = slasherViewModel.getRoomEffects("High School").firstOrNull() ?: listOf()
