...
class DashboardFragment : Fragment() {
    ...
    private fun handleSlasherClick(slasher: Slasher) {
        val action = when (slasher.locationType) {
            ...
            "Barn" -> R.id.action_dashboardFragment_to_leslieBarnFragment
            "Theater" -> R.id.action_dashboardFragment_to_bughuulTheaterFragment
            "Trap House" -> R.id.action_dashboardFragment_to_collectorTrapFragment
            else -> null
        }
                ...
                if (user != null) {
                    viewModel.slashers.collect { slasherList ->
                        ...
                        if (userId != null) {
                            ...
                            val collectorTask = viewModel.getActiveTask(userId, 11).firstOrNull()
                            if (collectorTask != null && !collectorTask.isCompleted) {
                                activeTaskLocations.add("Trap House")
                            } else {
                                viewModel.loadOrInsertNextTask(userId, 11)
                                val newTask = viewModel.getActiveTask(userId, 11).firstOrNull()
                                if (newTask != null && !newTask.isCompleted) {
                                    activeTaskLocations.add("Trap House")
                                }
                            }

                            val bughuulTask = viewModel.getActiveTask(userId, 10).firstOrNull()
                            if (bughuulTask != null && !bughuulTask.isCompleted) {
                                activeTaskLocations.add("Theater")
                            } else {
                                viewModel.loadOrInsertNextTask(userId, 10)
                                val newTask = viewModel.getActiveTask(userId, 10).firstOrNull()
                                if (newTask != null && !newTask.isCompleted) {
                                    activeTaskLocations.add("Theater")
                                }
                            }

                            val leslieTask = viewModel.getActiveTask(userId, 9).firstOrNull()
                            if (leslieTask != null && !leslieTask.isCompleted) {
                                activeTaskLocations.add("Barn")
                            } else {
                                viewModel.loadOrInsertNextTask(userId, 9)
                                val newTask = viewModel.getActiveTask(userId, 9).firstOrNull()
                                if (newTask != null && !newTask.isCompleted) {
                                    activeTaskLocations.add("Barn")
                                }
                            }
